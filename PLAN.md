...
| 1.3 测试 + CI             | ✅ 已完成 | Vitest 覆盖 13 个文件 / 62 项测试；CDP smoke 覆盖开发态与生产预渲染态                                |
...
一个 React 19 + Vite 8、纯静态、无后端的双语作品集模板：构建时预渲染 `/`、`/en/`、`/zh/` 的 Landing + About，浏览器再 hydrate 为完整 SPA；所有文案集中在 `src/data.js`，站内编辑结果存 `localStorage`，靠「导出代码 → 粘回源码 → git push」上线。当前已具备共享持久化层、62 项 Vitest、开发/生产浏览器 smoke、SEO/SSG、lint/format、CI、编辑器按需加载、图片 B3 与首轮无障碍修复；下一步是字体 A1 和移动端 Lighthouse 复测。详见 `CODEBASE_ANALYSIS.html`。
...
- 验收结果：13 个测试文件、62 项测试通过。GitHub Actions 顺序为 `install → lint → test → build → check:dist → format:check`。
...
- **默认路线（已定）**：图片 **B3** + a11y 已完成；下一 PR 做字体砍字重/`preload`/fallback metrics（决策 **A1**），随后量一次 Lighthouse。**仅当仍 < 90** 才升级到模板友好且带构建期依赖的 **A2（自动子集字体）/ B1（AVIF/WebP 管线）**；**排除 A2′ / B2**（手工方案，破模板属性）。
...
1. 完成字体 A1：精简字重、关键资源提示、fallback metrics。
2. 运行移动端 Lighthouse，记录 Performance / a11y / BP / SEO / CLS。
3. 仅在 Performance < 90 时评估 A2 自动字体子集与 B1 构建期现代图片。
