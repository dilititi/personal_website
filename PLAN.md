...
| 1.3 测试 + CI             | ✅ 已完成 | Vitest 覆盖 23 个文件 / 113 项测试；CDP smoke 覆盖开发态与生产预渲染态                                          |
...
| Phase 4 | P0–P2 + UX1 已实现   | 空白起点、目标预设、审计、单页调参、结构模板与就地编辑         | 让模板 5 分钟可上手    |

排序原则：地基、质量与发布链路已建立；Phase 4 已根据首轮 5 人反馈完成 UX 收口。下一步先做任务制复测和 Phase 2/3 外部验收，再决定是否启动 Prompt Builder 等 P3+。
...
- **任务制复测**：按 [`PHASE4-USER-RETEST.md`](./PHASE4-USER-RETEST.md) 邀请首轮用户实际完成“选结构 → 填核心信息 → 就地改内容 → 调参 → 审计与导出”，记录完成率、耗时、提示次数与错误路径。
...
3. **Phase 4 任务制复测**：执行 [`PHASE4-USER-RETEST.md`](./PHASE4-USER-RETEST.md)，覆盖 5 人、3 种模板和至少 2 位移动端用户。
