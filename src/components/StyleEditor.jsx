...
function isImageUrl(value) {
  ...
  return /\.(png|jpe?g|webp|gif|avif|svg)(\?.*)?$/i.test(url) || url.startsWith('/picture/') || url.startsWith('/photos/') || url.startsWith('/works/')
}
  ...
  return (
    <Field label={label}>
      <select className="ce-input se-select" value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((option) => (
          <option key={option.value} value={option.value}>{pick(option.label || option, lang)}</option>
        ))}
      </select>
    </Field>
  )
        ...
        {Object.entries(STYLE_PRESETS).map(([id, preset]) => {
          ...
          return (
            ...
            >
              ...
              >
                <i className="se-preset-thumb-top"><b /><b /></i>
                <i className="se-preset-thumb-title">Aa</i>
                <i className="se-preset-thumb-card"><b /><b /><b /></i>
...
function ReferencePanel({ style, setStyle, lang }) {
  ...
  return (
    <div className="se-panel se-reference-panel">
      <p className="ce-hint">
        {lang === 'zh'
          ? '参考锚点先保存 mood keywords 和 visual anchors。当前版本不会自动推导 8 个维度；后续 AI 解析参考图时会把这些信号转成初始风格值。'
          : 'Reference anchors store mood keywords and visual anchors. This version does not infer the 8 dimensions yet; later AI parsing can turn these signals into initial style values.'}
      </p>

      <section className="se-reference-section">
        <div className="se-reference-head">
          <strong>{lang === 'zh' ? 'Mood keywords' : 'Mood keywords'}</strong>
          <span>{lang === 'zh' ? '描述你希望网站散发的情绪，不直接改 CSS。' : 'Describe the feeling before it becomes CSS.'}</span>
        </div>
        ...
        <div className="se-token-row">
          {mood.length === 0 && <span className="se-empty">{lang === 'zh' ? '还没有 mood keyword' : 'No mood keywords yet'}</span>}
          {mood.map((item) => (
        ...
        <div className="se-inline-add">
          <input
            ...
            type="text"
            value={keyword}
            placeholder={lang === 'zh' ? '自定义关键词，例如 nostalgic' : 'Custom keyword, e.g. nostalgic'}
      ...
      <section className="se-reference-section">
        <div className="se-reference-head">
          ...
          <span>{lang === 'zh' ? '记录图片、电影、网站、物件或文字参考。' : 'Capture images, films, sites, objects, or text references.'}</span>
        </div>
        <div className="se-anchor-builder">
          <SelectControl label={lang === 'zh' ? '类型' : 'Type'} value={draft.type} options={STYLE_ANCHOR_TYPES} onChange={(type) => setDraft((prev) => ({ ...prev, type }))} lang={lang} />
          <Field label={lang === 'zh' ? '标题' : 'Title'}>
            <input className="ce-input" type="text" value={draft.title} onChange={(e) => setDraft((prev) => ({ ...prev, title: e.target.value }))} />
          </Field>
          <Field label={lang === 'zh' ? 'URL / 路径' : 'URL / path'}>
            <input className="ce-input" type="text" value={draft.url} placeholder="/photos/reference.jpg" onChange={(e) => setDraft((prev) => ({ ...prev, url: e.target.value }))} />
          </Field>
          <RangeControl label={lang === 'zh' ? '权重' : 'Weight'} min={0} max={1} step={0.01} value={draft.weight} onChange={(weight) => setDraft((prev) => ({ ...prev, weight }))} />
          <Field label={lang === 'zh' ? '提示' : 'Note'}>
            <textarea className="ce-input se-anchor-note" value={draft.note} onChange={(e) => setDraft((prev) => ({ ...prev, note: e.target.value }))} />
          </Field>
        ...
        <div className="se-anchor-list">
          {anchors.length === 0 && <div className="se-empty se-anchor-empty">{lang === 'zh' ? '还没有 visual anchor' : 'No visual anchors yet'}</div>}
            ...
            return (
              <article key={anchor.id || `${anchor.title}-${index}`} className="se-anchor-card">
                <span
                  ...
                  style={hasImage ? { backgroundImage: `url("${String(anchor.url).replace(/"/g, '\\"')}")` } : undefined}
                >
                  {!hasImage && <b>{pick(type?.label, lang) || anchor.type || 'ref'}</b>}
                </span>
                <div className="se-anchor-copy">
                  <strong>{anchor.title || anchor.url || (lang === 'zh' ? '未命名参考' : 'Untitled anchor')}</strong>
                ...
                <button className="se-anchor-remove" type="button" onClick={() => removeAnchor(index)} aria-label={lang === 'zh' ? '删除参考' : 'Remove anchor'}>
                  x
                </button>
              </article>
            )
...
function ColorPanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      <ColorControl label={lang === 'zh' ? '背景' : 'Background'} value={color.background} onChange={(background) => update({ background })} />
      <ColorControl label={lang === 'zh' ? '表面' : 'Surface'} value={color.surface} onChange={(surface) => update({ surface })} />
      <ColorControl label={lang === 'zh' ? '正文' : 'Text'} value={color.text} onChange={(text) => update({ text })} />
      <ColorControl label={lang === 'zh' ? '弱文本' : 'Muted'} value={color.muted} onChange={(muted) => update({ muted })} />
      <ColorControl label={lang === 'zh' ? '主色' : 'Primary'} value={color.primary} onChange={(primary) => update({ primary })} />
      <ColorControl label={lang === 'zh' ? '辅色' : 'Secondary'} value={color.secondary} onChange={(secondary) => update({ secondary })} />
      <RangeControl label={lang === 'zh' ? '对比度' : 'Contrast'} min={0} max={1} step={0.01} value={color.contrast} onChange={(contrast) => update({ contrast })} />
      <RangeControl label={lang === 'zh' ? '饱和度' : 'Saturation'} min={0} max={1} step={0.01} value={color.saturation} onChange={(saturation) => update({ saturation })} />
      <RangeControl label={lang === 'zh' ? '冷暖' : 'Temperature'} min={-1} max={1} step={0.01} value={color.temperature} onChange={(temperature) => update({ temperature })} />
    </div>
  )
}
...
function TypographyPanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      ...
      <SelectControl label={lang === 'zh' ? '标题字体' : 'Display font'} value={typography.display} options={FONT_OPTIONS} onChange={(display) => update({ display })} lang={lang} />
      <SelectControl label={lang === 'zh' ? '正文字体' : 'Body font'} value={typography.body} options={FONT_OPTIONS} onChange={(body) => update({ body })} lang={lang} />
      <RangeControl label={lang === 'zh' ? '字号比例' : 'Type scale'} min={1.1} max={1.7} step={0.001} value={typography.scale} onChange={(scale) => update({ scale })} />
      <RangeControl label={lang === 'zh' ? '字距' : 'Tracking'} min={-30} max={60} step={1} value={typography.tracking} onChange={(tracking) => update({ tracking })} />
    </div>
  )
}
...
function SpacePanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      <RangeControl label={lang === 'zh' ? '密度' : 'Density'} min={0} max={1} step={0.01} value={space.density} onChange={(density) => update({ density })} />
      <RangeControl label={lang === 'zh' ? '圆角' : 'Radius'} min={0} max={24} step={1} suffix="px" value={space.radius} onChange={(radius) => update({ radius })} />
      <RangeControl label={lang === 'zh' ? '章节呼吸' : 'Section rhythm'} min={64} max={140} step={1} suffix="px" value={space.sectionGap} onChange={(sectionGap) => update({ sectionGap })} />
      <RangeControl label={lang === 'zh' ? '卡片间距' : 'Card gap'} min={10} max={36} step={1} suffix="px" value={space.cardGap} onChange={(cardGap) => update({ cardGap })} />
    </div>
  )
}
...
function MotionPanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      <SelectControl label={lang === 'zh' ? '动态模式' : 'Motion mode'} value={motion.mode} options={MOTION_OPTIONS} onChange={(mode) => update({ mode })} lang={lang} />
      <SelectControl label={lang === 'zh' ? '缓动曲线' : 'Easing'} value={motion.easing} options={EASING_OPTIONS} onChange={(easing) => update({ easing })} lang={lang} />
      <RangeControl label={lang === 'zh' ? '速度' : 'Duration'} min={80} max={800} step={10} suffix="ms" value={motion.duration} onChange={(duration) => update({ duration })} />
    </div>
  )
}
...
function TexturePanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      <RangeControl label={lang === 'zh' ? '\u9897\u7c92\u8986\u5c42' : 'Grain overlay'} min={0} max={0.32} step={0.01} value={texture.grain} onChange={(grain) => update({ grain })} />
      <RangeControl label={lang === 'zh' ? '\u5f71\u50cf\u9971\u548c' : 'Image saturation'} min={0.4} max={1.8} step={0.01} value={texture.imageSaturation} onChange={(imageSaturation) => update({ imageSaturation })} />
      <RangeControl label={lang === 'zh' ? '\u5f71\u50cf\u5bf9\u6bd4' : 'Image contrast'} min={0.7} max={1.5} step={0.01} value={texture.imageContrast} onChange={(imageContrast) => update({ imageContrast })} />
      <RangeControl label={lang === 'zh' ? '\u5f71\u50cf\u4eae\u5ea6' : 'Image brightness'} min={0.7} max={1.3} step={0.01} value={texture.imageBrightness} onChange={(imageBrightness) => update({ imageBrightness })} />
    </div>
  )
}
...
function LightPanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      <RangeControl label={lang === 'zh' ? '\u6295\u5f71\u6df1\u5ea6' : 'Shadow depth'} min={0} max={1} step={0.01} value={light.shadowDepth} onChange={(shadowDepth) => update({ shadowDepth })} />
      <RangeControl label={lang === 'zh' ? '\u9634\u5f71\u67d4\u548c' : 'Shadow softness'} min={0} max={1} step={0.01} value={light.shadowSoftness} onChange={(shadowSoftness) => update({ shadowSoftness })} />
      <RangeControl label={lang === 'zh' ? '\u9ad8\u5149\u5f3a\u5ea6' : 'Highlight'} min={0} max={1} step={0.01} value={light.highlight} onChange={(highlight) => update({ highlight })} />
    </div>
  )
}
...
function DepthPanel({ style, updateDimension, lang }) {
  ...
  return (
    <div className="se-panel se-grid">
      <RangeControl label={lang === 'zh' ? '\u80cc\u666f\u6a21\u7cca' : 'Background blur'} min={0} max={16} step={1} suffix="px" value={depth.blur} onChange={(blur) => update({ blur })} />
      <RangeControl label={lang === 'zh' ? '\u73bb\u7483\u611f' : 'Glass'} min={0} max={1} step={0.01} value={depth.glass} onChange={(glass) => update({ glass })} />
      <RangeControl label={lang === 'zh' ? '\u5c42\u6b21\u4f4d\u79fb' : 'Parallax shift'} min={0} max={1} step={0.01} value={depth.parallax} onChange={(parallax) => update({ parallax })} />
    </div>
  )
}
...
function ReservedPanel({ dimension, lang }) {
  return (
    <div className="se-panel se-reserved-panel">
      <p className="ce-hint">
        {lang === 'zh'
          ? `${pick(dimension.label, lang)} 已经进入 STYLE 数据结构，但第一刀先只开放色彩、排版、空间、动态。`
          : `${pick(dimension.label, lang)} already exists in the STYLE data shape. The first slice exposes Color, Type, Space, and Motion first.`}
      </p>
      <div className="se-reserved-card">
        <strong>{pick(dimension.label, lang)}</strong>
        <span>{dimension.summary}</span>
        <em>{lang === 'zh' ? '下一阶段会在这里放可调控件。' : 'Controls for this dimension will land in the next phase.'}</em>
      </div>
    </div>
  )
}
...
export default function StyleEditor({ open, onClose }) {
  ...
  const [sideWidth, setSideWidth] = useState(() => {
    try { return Number(localStorage.getItem('chen.se.sideWidth')) || 520 } catch { return 520 }
  })
  ...
  useEffect(() => { try { localStorage.setItem('chen.se.mode', mode) } catch {} }, [mode])
  useEffect(() => { try { localStorage.setItem('chen.se.sideWidth', String(sideWidth)) } catch {} }, [sideWidth])
    ...
    >
      <div className="ce-shell se-shell">
        ...
        <header className="ce-header">
          <div className="ce-title">
            ...
            <span className="ce-title-sub">
              {mode === 'side'
                ? (lang === 'zh' ? '侧栏模式 - 左侧主站实时预览' : 'Side mode - live preview on the left')
                : (lang === 'zh' ? '全屏模式 - 可切换侧栏直接观察网站' : 'Modal mode - switch to side for live site preview')}
            </span>
          </div>
          <div className="ce-header-actions">
            {isCustomized && <span className="ce-tag">{lang === 'zh' ? '本地风格中' : 'local style'}</span>}
            <button
            ...
            <button className="ce-close" type="button" onClick={onClose} aria-label="close">x</button>
          </div>
        </header>
        ...
        <div className="ce-body se-body">
          <nav className="ce-tabs" aria-label="style dimensions">
            {tabs.map((tab) => (
              ...
              >
                ...
                {!tab.live && <span className="ce-tab-dot" title={lang === 'zh' ? '已预留' : 'Reserved'}>○</span>}
              </button>
            ))}
          </nav>
          ...
          <main className="ce-main">
            <div className="ce-main-head">
              ...
              {!activeTab.live && <span className="ce-tag">{lang === 'zh' ? '已预留' : 'reserved'}</span>}
            </div>
        ...
        <footer className="ce-footer">
          <div className={`ce-footer-msg ${storageError ? 'ce-footer-error' : ''}`}>{storageError || copied}</div>
          <div className="ce-footer-actions">
            <button className="ce-btn ce-btn-ghost" type="button" onClick={resetStyle} disabled={!isCustomized}>
