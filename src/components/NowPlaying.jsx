...
export default function NowPlaying({ layout = 'default' }) {
  ...
  const onPointerDown = e => {
    ...
    if (
      e.target.closest('.np-list, .np-player, iframe, audio, input, select, textarea, .np-list-x')
    )
      return
  ...
  const positionStyle = pos ? { left: pos.x, top: pos.y, bottom: 'auto', right: 'auto' } : undefined
  ...
  return (
    ...
    >
      <div className="np-pill-wrap">
        ...
        {active && (
          <button
            className="np-pill-stop"
            onClick={stop}
            aria-label="stop"
            title={lang === 'zh' ? '停止播放' : 'Stop playback'}
          >
            ■
          </button>
        )}
      ...
      <div className="np-open-wrap">
        <button className="np-close" onClick={() => setCollapsed(true)} aria-label="collapse">
          −
        </button>
        ...
        {source === 'html5' && (
          <>
            ...
            <input
              ...
              multiple
              onChange={e => {
                addUploads(e.target.files, { autoplay: false })
                e.target.value = ''
              }}
              style={{ display: 'none' }}
            />
          ...
          <div className="np-list" role="list">
            {list.map((tr, j) => {
              ...
              return (
                <div
                  key={key}
                  className={`np-list-row ${sel ? 'sel' : ''} ${playing ? 'playing' : ''}`}
                  role="listitem"
                >
                  <button className="np-list-item" onClick={() => playFromList(j)}>
                    ...
                    {playing && (
                      <span className="np-list-eq" aria-hidden="true">
                        <i />
                        <i />
                        <i />
                      </span>
