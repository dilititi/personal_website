...
export function deriveStyleVars(style) {
  ...
  const sectionY = Math.round(sectionGap * (1.45 - density * 0.45))
  const sectionX = Math.round(80 * (1.15 - density * 0.25))
  const headerGap = Math.round(sectionGap * (0.85 - density * 0.5))
  const imageSatValue = Number(((0.75 + saturation * 0.8) * imageSaturation).toFixed(3))
  const imageContrastValue = Number(((0.86 + contrast * 0.26) * imageContrast).toFixed(3))
  ...
  return {
    '--ink-void': background,
    '--ink-deep': surface,
    '--ink-soft': mixHex(surface, text, 0.06 + contrast * 0.04),
    '--ink-line': mixHex(surface, text, 0.12 + contrast * 0.1),
    '--ink-haze': mixHex(surface, text, 0.22 + contrast * 0.14),
    '--cream': text,
    ...
    '--style-card-gap': `${Math.round(cardGap)}px`,
    '--style-type-scale': String(scale),
